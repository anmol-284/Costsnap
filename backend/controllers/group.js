    ...
    try{
        ...
        for(let i = 0; i < grp.grpMembers.length; i++){                  // row-wise
            if(grp.grpMembers[i].username !== paidby){
                for(let j = 0; j < grp.grpMembers[i].settlement.length; j++){
                    if(grp.grpMembers[i].settlement[j].username === paidby){
                        grp.grpMembers[i].settlement[j].share += share[i].shareamount;
                    }
                }
            }else{
                let k = 0;
